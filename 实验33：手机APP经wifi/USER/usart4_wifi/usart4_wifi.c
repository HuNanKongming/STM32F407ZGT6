...
unsigned char start[] = "AT+RST\r\n";
unsigned char Router[]="AT+CWJAP=\"HUAWEI\",\"11112222\"";  //连接wifi 名字为qiming_wifi 密码0123456789
...
unsigned char connect[]="AT+CIPSTART=\"TCP\",\"10.122.128.197\",8080";
...
{
			GPIO_SetBits(GPIOE,GPIO_Pin_14);                    //PF0控制抽水泵的开关
			GPIO_ResetBits(GPIOE,GPIO_Pin_11);
}
void water_suction_pump_close()
{
			GPIO_ResetBits(GPIOE,GPIO_Pin_14);                    //PF0控制抽水泵的开关
			GPIO_ResetBits(GPIOE,GPIO_Pin_11);
}
...
void WIFI_Server_Init(void)
{
	...
	delay_ms(1000);
	uart4SendChars(start,sizeof(start));   
	delay_ms(1000);
	delay_ms(1000);
	delay_ms(1000);
	delay_ms(1000);
	...
	delay_ms(1000);                          
	uart4SendChars(connect,sizeof(connect));
	delay_ms(1000);  
...
void UART4_IRQHandler(void)  
{
	...
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收中断 
		{
				rec_data =(u8)USART_ReceiveData(UART4);         //(USART4->DR) 读取接收到的数据
        ...
        if(rec_data=='S')		  	                         //如果是S，表示是命令信息的起始位
				...
				{
					if(strcmp("open_led1",(char *)receive_str)==0)         LED0=0;	   //点亮LED0
					...
					else if(strcmp("noodlesON",(char *)receive_str)==0)
            {
								LED0 = 0;
              ...
              GPIO_ResetBits(GPIOD,GPIO_Pin_13);
					...
					else if(strcmp("noodlesON_ban",(char *)receive_str)==0)
            {
								LED0 = 0;
								GPIO_SetBits(GPIOD,GPIO_Pin_12);
								GPIO_ResetBits(GPIOD,GPIO_Pin_13);
						...
						else if(strcmp("noodlesOFF",(char *)receive_str)==0)
						{
								LED0 = 1;
...
//				//揉面机控制
//							 else if(strcmp("打开揉面机",(char *)receive_str)==0)
//							{
//									//对揉面机进行控制
//								rou_mianji_open();
//							}
//							 else if(strcmp("关闭揉面机",(char *)receive_str)==0)
//							{
//									//对揉面机进行控制
//								rou_mianji_close();
//							}
