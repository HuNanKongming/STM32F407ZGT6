...
u8 receive_str4[UART4_REC_NUM];     //接收缓存数组,最大USART_REC_LEN个字节 
u8 uart_byte_count4=0;
...
void UART4_IRQHandler(void)  
{
	...
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收中断 
        ...
        if(rec_data=='S')		  	                         //如果是S，表示是命令信息的起始位
				{
					uart_byte_count4=0x01; 
				}

			else if(rec_data=='E')		                         //如果E，表示是命令信息传送的结束位
				{
					if(strcmp("open_led1",(char *)receive_str4)==0)         LED0=0;	   //点亮LED0
					else if(strcmp("close_led1",(char *)receive_str4)==0)   LED0=1;	   //关灭LED0
					
					else if(strcmp("open_led2",(char *)receive_str4)==0)    LED1=0;	   //点亮LED1
					else if(strcmp("close_led2",(char *)receive_str4)==0)   LED1=1;	   //关灭LED1
					
					else if(strcmp("open_led3",(char *)receive_str4)==0)    LED2=0;	   //点亮LED2
					else if(strcmp("close_led3",(char *)receive_str4)==0)   LED2=1;	   //关灭LED2
					
					else if(strcmp("open_beep",(char *)receive_str4)==0)    BEEP=1; 	   //蜂鸣器响
					else if(strcmp("close_beep",(char *)receive_str4)==0)   BEEP=0; 	   //蜂鸣器不响
          if(strcmp("noodlesON",(char *)receive_str4)==0)
            {
								LED0 = 0;
            }

						else if(strcmp("noodlesOFF",(char *)receive_str4)==0)
						{
								LED0 = 1;
						}
						...
						//揉面机控制
					 else if(strcmp("kneadON",(char *)receive_str4)==0)
					{
							//对揉面机进行控制
						LED1 = 0;
					}
					 else if(strcmp("kneadOFF",(char *)receive_str4)==0)
					{
							//对揉面机进行控制
						LED1 = 1;
					}
					...
					//抽水泵控制
					 else if(strcmp("pumpON",(char *)receive_str4)==0)
					{
							//对揉面机进行控制
								LED2 = 0;
					}
					 else if(strcmp("pumpOFF",(char *)receive_str4)==0)
					{
							//对揉面机进行控制
							LED2 = 1;
					}
					...
					else if(strcmp("app_mode1",(char *)receive_str4)==0)    
					    {
								APP_mode=0; 
					...
					else if(strcmp("app_mode2",(char *)receive_str4)==0)    
					    {
								APP_mode=1;
				...
				//揉面机控制
							 else if(strcmp("打开揉面机",(char *)receive_str4)==0)
							{
									//对揉面机进行控制
								rou_mianji_open();
							}
							 else if(strcmp("关闭揉面机",(char *)receive_str4)==0)
							{
									//对揉面机进行控制
								rou_mianji_close();
							}
					...
					else if(receive_str4[0]=='T')   //时间校准
					    {						
								RTC_DateStruct.RTC_Year= (receive_str4[3]-'0')*10 + (receive_str4[4]-'0');						
								RTC_DateStruct.RTC_Month= (receive_str4[5]-'0')*10 + (receive_str4[6]-'0');							
								RTC_DateStruct.RTC_Date= (receive_str4[7]-'0')*10 + (receive_str4[8]-'0');							
								RTC_TimeStruct.RTC_Hours= (receive_str4[9]-'0')*10 + (receive_str4[10]-'0');						
								RTC_TimeStruct.RTC_Minutes= (receive_str4[11]-'0')*10 + (receive_str4[12]-'0');					
								RTC_TimeStruct.RTC_Seconds= (receive_str4[13]-'0')*10 + (receive_str4[14]-'0');		
				  ...
				  if(APP_mode==1)   //APP为接收发送区时，显示APP发来的数据  数据一定要以S开头 E结尾
					  {
					    LCD_DisplayString(30,190,16,receive_str4);	 //可以根据自己发送的数据 写相应的动作 按前面的编写方法
					  }
						
					for(uart_byte_count4=0;uart_byte_count4<32;uart_byte_count4++)receive_str4[uart_byte_count4]=0x00;
					uart_byte_count4=0;    
				}				  
			else if((uart_byte_count4>0)&&(uart_byte_count4<=UART4_REC_NUM))
				{
				   receive_str4[uart_byte_count4-1]=rec_data;
				   uart_byte_count4++;
				}                		 
   } 
} 
