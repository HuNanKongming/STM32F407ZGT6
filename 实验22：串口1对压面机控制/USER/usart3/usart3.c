...
u8 receive_str3[USART3_REC_NUM];     //接收缓存数组,最大USART_REC_LEN个字节 
...
****************************************************************************/
int fputc3(int ch, FILE *f)   //重定向，让printf输出到串口  
{
    uart3SendChar(ch);
...
void USART3_IRQHandler(void)  
{
	...
	{
		//LED1 = 0;
		rebuf3[i++]=USART_ReceiveData(USART3);//读取串口数据，同时清接收标志

		if (rebuf3[0]!=0x5a)//帧头不对
			i=0;	
		if ((i==2)&&(rebuf3[1]!=0x5a))//帧头不对
			i=0;
	
		if(i>3)//i等于4时，已经接收到数据量字节rebuf3[3]
		{
			if(i!=(rebuf3[3]+5))//判断是否接收一帧数据完毕
				return;	
			switch(rebuf3[2])//接收完毕后处理
			{
				case 0x45:
					if(!Receive_ok3)//当数据处理完成后才接收新的数据
					{
						memcpy(TEMP_data3,rebuf3,9);//拷贝接收到的数据
				/*for(int j=0;j < 9;j++)
		{
			printf("%c",rebuf3[j]);
		}
						*/
						Receive_ok3=1;//接收完成标志
					}
					break;
				case 0x15:break;
				case 0x35:break;
